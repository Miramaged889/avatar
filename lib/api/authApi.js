 ...
 * @param {FormData|Object} profileData - FormData for file upload or object for JSON
 */
export const updateSuperuserMe = async (profileData) => {
  try {
    let config = {};
    
    // If FormData, let axios set Content-Type automatically (it will be multipart/form-data with boundary)
    // We don't manually set Content-Type for FormData to allow axios to set the boundary
    ...
    const response = await apiClient.patch("/api/superuser/me/", profileData, config);
    return {
      success: true,
      data: response.data,
    };
